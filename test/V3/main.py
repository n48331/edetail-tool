...
for filename in os.listdir(pdf_path):
    if filename.endswith(".pdf"):
        ...
        break
log(f'pdf folder with pdf : {pdf != ""} ({pdf_filename})',2 if pdf != "" else 1)
log(f'source_folder : {os.path.exists(source_folder)} ({source_folder})',2 if os.path.exists(source_folder) else 1)
log(f'image folder : {os.path.exists(image_folder)} ({image_folder})',2 if os.path.exists(image_folder) else 1)
log(f'shared_src : {os.path.exists(shared_src)} ({shared_src})',2 if os.path.exists(shared_src) else 1)
if pdf =='' or not os.path.exists(shared_src) or not os.path.exists(image_folder) or not os.path.exists(source_folder):
...
log('Total proccess time: '+"%s seconds." % (time.time() - start_time))
